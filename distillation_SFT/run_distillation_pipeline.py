...
================================================
- KL Regularization: Prevents catastrophic forgetting of base model capabilities
- Early Stopping: Prevents overfitting on small datasets
- Quality Filtering: Filters out degenerate/repetitive samples
- Single Consistent Template: Avoids gradient conflicts from template randomization
- Low Learning Rate (2e-5) with Cosine Schedule: Stable training
...
Usage:
    ...
    python run_distillation_pipeline.py --mode train --use_kl_regularization  # Recommended!
"""
...
def print_banner():
    ...
    banner = """
╔══════════════════════════════════════════════════════════════════╗
║          Reasoning Distillation Pipeline v2.0                   ║
║          Following DeepSeek-R1-Distill Methodology              ║
╠══════════════════════════════════════════════════════════════════╣
║  Dataset: OpenThoughts-114k                                      ║
║  Method: SFT with KL Regularization + Early Stopping            ║
║  Key Learning: Quality over Quantity, Prevent Forgetting        ║
╚══════════════════════════════════════════════════════════════════╝
    """
    print(banner)
...
def run_full_pipeline(args):
    ...
    # Configure training with improvements from experiments
    config = DistillationConfig(
        ...
        device=device,
        # New improvements from experiments
    ...
    logger.info("="*60)
    logger.info("TRAINING CONFIGURATION (with experiment learnings)")
    logger.info("="*60)
    logger.info(f"  Model: {config.base_model_name}")
    logger.info(f"  Learning Rate: {config.learning_rate}")
    logger.info(f"  Epochs: {config.num_train_epochs}")
    logger.info(f"  KL Regularization: {config.use_kl_regularization}")
...
Examples:
  ...
  python run_distillation_pipeline.py --mode all --max_samples 100 --epochs 1
    ...
    # KL Regularization (requires 2x memory for reference model)
    parser.add_argument(
        "--use_kl_regularization",
        action="store_true",
        default=False,
        help="Use KL regularization to prevent forgetting (requires 2x GPU memory)"
    )
    
    parser.add_argument(
        "--no_kl_regularization",
        action="store_true",
        help="Disable KL regularization (default)"
    )
    ...
    # Early Stopping (RECOMMENDED from experiments)
    parser.add_argument(
        "--early_stopping_patience",
        type=int,
        default=3,
        help="Early stopping patience epochs (default: 3)"
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.01,
        help="Early stopping improvement threshold (default: 0.01)"
    )
    
    # Gradient Clipping (RECOMMENDED from experiments)
    parser.add_argument(
        "--max_grad_norm",
        type=float,
        default=1.0,
    ...
    parser.add_argument(
        ...
        default=None,
        choices=["cuda", "cpu"],
        help="Force device (default: auto-detect). Use 'cpu' to bypass CUDA issues."
    )
    ...
    # Handle no_kl_regularization flag
    if args.no_kl_regularization:
        args.use_kl_regularization = False
